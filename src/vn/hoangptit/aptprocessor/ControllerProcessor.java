...
@SupportedAnnotationTypes(value = { "vn.hoangptit.ann.Controller" })
		...
		for (TypeElement ann : annotations) {
			...
			for (Element e2 : e2s) {
				...
				if (e2.getKind() != ElementKind.CLASS) {
					DevLog.log("           - Error!!!");
					messager.printMessage(Kind.ERROR, "@Controller using for class only ", e2);
				} else {
					...
					if (!className.endsWith("Controller")) {
						DevLog.log("           - Error!!!");
						messager.printMessage(Kind.ERROR, "Class using @Controller must have suffix Controller", e2);
					}
				}
			}
